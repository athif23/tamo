...
**Eleventh bounded slice: package-manager-agnostic pack and flexible Behavior entrypoint.** `tamo pack` no longer gates on package-manager identity: any Node project with a valid package.json is packable regardless of the `packageManager` field or which lockfiles are present. Pack still never modifies the source, still strips source `name`/`version`, still preserves `packageManager` when present and never invents it, still excludes lockfiles, still stays Behavior-blind, and keeps all secret/private/generated-file, explicit-`--include`, and dry-run/review semantics. No detection framework, active-package-manager concept, lockfile precedence, inference, or prompt is introduced. `recipe.json` gains one optional key, `"behavior"`, holding a single relative `.mjs` path under the recipe root (for example `"setup.mjs"` or `"scripts/setup.mjs"`). When absent, `behavior.mjs` remains the default optional entrypoint; when present, that file alone is the Behavior entrypoint. The value must be a non-empty string, relative, conservatively normalized, staying inside the recipe directory, and ending in `.mjs`; missing explicitly referenced files, non-string/empty values, unsupported extensions, absolute paths, escapes, and symlinked entrypoints fail loudly per selected recipe only. The selected custom file is fingerprinted as a reviewed input exactly like `behavior.mjs`, and the `prepare`/`finalize`/`verify` runtime contract is unchanged. Create-time dependency installation remains pnpm-based for now; this slice does not add npm/Bun/Yarn installation.
