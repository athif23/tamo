...
import { versions } from "../src/features/effect-oxlint.ts";
...
try {
  ...
  // Round trip: pack a sample project into an isolated Tamo home, then create a
  // new ordinary project from the preset with a real install. The home is
  // forced into the environment so CLI children never touch the real one.
  ...
  assert.deepEqual(
    packed.preset.files.map((file: { path: string }) => file.path),
    ["tsconfig.json", "src/utils/cn.ts", "src/utils/result.ts", ".env.example"],
  );
  ...
  console.log("Integration passed: pack → preset → create round trip with real install.");
} finally {
